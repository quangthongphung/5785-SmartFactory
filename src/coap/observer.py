...
class FactoryObserver:
    ...
    async def observe_resource(self, uri: str) -> None:
        ...
        task = asyncio.create_task(observe_loop())
    ...
    def _handle_notification(self, uri: str, response: Message) -> None:
        ...
        if last is not None:
            if seq <= last and (last - seq) < (2 ** 23):
                ...
                log.warning(
                    f"STALE notification on {uri}: seq={seq} <= last={last}"
                )
                return
    ...
    async def fetch_manifest(self) -> None:
        ...
        if response.opt.block2 is not None:
            log.info(f"Block2 option: {response.opt.block2}")
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            await asyncio.gather(
                self.observe_resource(uris[0]),
                self.observe_resource(uris[1])
            )
